...
if __name__ == '__main__':
    ...
    prs.add_argument("-route", dest="route", type=str, default='nets/2way-single-intersection/london.rou.xml', help="Route definition xml file.\n")
    ...
    out_csv = 'outputs/QL-{}_alpha{}_gamma{}_eps{}_decay{}_reward{}'.format(experiment_time, args.alpha, args.gamma, args.epsilon, args.decay, args.reward)

    env = SumoEnvironment(net_file='nets/2way-single-intersection/london.net.xml',
    ...
    for run in range(1, args.runs + 1):
        ...
        done = {'__all__': False}  # Reset done for each run
        ...
        if args.fixed:
            ...
            while not done['__all__']:
                ...
                s, r, done, truncated, _ = env.step(actions)

                print(f"Run {run}: Step {env.sim_step} rewards = {r}")

                for agent_id in ql_agents.keys():
                    next_state = env.encode(s[agent_id], agent_id)
                    ql_agents[agent_id].learn(next_state=next_state, reward=r[agent_id])


                print(f"Run {run}: Step {env.sim_step} completed, done = {done}")
