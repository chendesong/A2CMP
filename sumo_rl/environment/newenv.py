...
from .newsignal import NewSignal
...
class NewEnv(MultiAgentEnv):
    ...
    def __init__(self, net_file, route_file, out_csv_name=None, use_gui=False, num_seconds=20000,
        ...
        self.traffic_signals = {
            ts: NewSignal(self, ts, self.delta_time, self.yellow_time, self.min_green, self.max_green) for ts in
            self.ts_ids}
        self.vehicles = dict()
    ...
    def reset(self, seed=None, options=None, **kwargs):
        ...
        self.traffic_signals = {
            ts: NewSignal(self, ts, self.delta_time, self.yellow_time, self.min_green, self.max_green)
    ...
    def step(self, action):
        ...
        try:
            ...
            # terminated 和 truncated 标志
            terminated = {'__all__': self.sim_step > self.sim_max_time}
            truncated = {'__all__': False}  # 根据具体需求设置截断标志

            if self.single_agent:
                return observations[self.ts_ids[0]], rewards[self.ts_ids[0]], terminated['__all__'], truncated[
                    '__all__'], {}
            else:
    ...
    def _apply_actions(self, actions):
        if self.single_agent:
            print(f"Applying action for single agent: {actions}")
            self.traffic_signals[self.ts_ids[0]].set_next_phase(actions)
        else:
            print(f"Applying actions for multiple agents: {actions}")
